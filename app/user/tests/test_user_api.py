...
class PublicUserApiTest(TestCase):
    ...
    def test_returns_400_if_user_already_exists(self):
        payload = {"email": "test@test.com", "password": "irrelevant", "name": "name"}
        create_user(**payload)
    ...
    def test_rejects_short_passwords(self):
        payload = {"email": "test@test.com", "password": "p", "name": "name"}
        res = self.client.post(CREATE_USER_URL, payload)
...
class PrivateUserApiTest(TestCase):
    def setUp(self):
        self.user = create_user(
            email="some@email.com", password="irrelevant", name="some-name"
        )
        self.client = APIClient()
    ...
    def test_updates_email_when_sent(self):
        ...
        self.assertFalse(get_user_model().objects.filter(email=old_email).exists())
        self.assertTrue(get_user_model().objects.filter(email=new_email).exists())
